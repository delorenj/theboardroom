...
import { Application, Container, Graphics, Sprite, Text, TextStyle, Texture, BlurFilter, Assets } from 'pixi.js';
...
export class BoardroomScene2D {
  ...
  private animate(): void {
    // Pulse effect for speaking participant
    for (const [, participant] of this.participants) {
      if (participant.isSpeaking) {
        const pulse = Math.sin(Date.now() * 0.005) * 0.1 + 0.9;
        participant.glowEffect.alpha = pulse;
      }
    }
